...
const updateNotes = async (req, res) => {
    try {
        ...
        if (!updated) { res.status(404).json({ message: 'notes not found!' }) }

        res.status(200).json({ message: 'updated sucessfully', data: updated })
    } catch (error) {
...
const deleteNotes = async (req, res) => {
  try {
    const deleted = await notesModel.findbyIdAndUpdate(req.params.id)

    if (!deleted)
        { res.status(404).json({message:'notes not found'})}
    res.status(200).json({message: 'notes deleted sucesssfully'})
}
catch (error) {
